...
def _bucket(b):
    a = _area(b)
    if a < 10000: return "small"
    if a < 250000: return "medium"
    return "large"
...
def summarize(results):
    ...
    by_type = {"text":0, "icon":0}; cnt_type={"text":0,"icon":0}
    by_bucket = {"small":0,"medium":0,"large":0}; cnt_bucket={"small":0,"medium":0,"large":0}
    for r in results:
        tt = r["target_type"]; cnt_type[tt]+=1; by_type[tt]+=1 if r["success"] else 0
        b = _bucket(r["gold_box"]); cnt_bucket[b]+=1; by_bucket[b]+=1 if r["success"] else 0
